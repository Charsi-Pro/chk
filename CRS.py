...
class menu:
	...
	def main(self):
		...
		try:
			...
			print(' [%s!%s] Login failed ...'%(M,N));os.system('rm -rf token.x');time.sleep(2);login().__login__()
		...
		jalan(' %s[ %Happy Crack Bang %s%s ]'%(N,H,r,N))
	...
	def pilih(self):
		...
		if usna in ['']:
			print(' %s'%(N))
			print(' %s[%s!%s] don,t empty anj'%(N,M,N));time.sleep(2);menu().main()
		elif usna in ['1','01']:
			try:
			...
			for ikeh in range(pler):
				...
				try:
					print(' %s'%(N))
					idt = input(' %s[%s•%s] Enter the id to %s : '%(N,O,N,ikeh))
